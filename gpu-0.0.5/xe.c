...
#if 0
	/* if you want more texture ram, boot with mem=128M and use this */
#define RINGBUFFER_BASE 0x08000000
#define RINGBUFFER_SIZE 0x17C00000
#else
#define RINGBUFFER_BASE 0x1e000000
#define RINGBUFFER_SIZE 0x01C00000
#endif
...
void Xe_pMasterInit(struct XenosDevice *xe, u32 buffer_base)
{
	...
	if (!f)
		Xe_Fatal(xe, "ucode0.bin %m");

	fread(xe->ucode0, 0x120*4, 1, f);
	fclose(f);

	f = fopen("ucode1.bin", "rb");
	if (!f)
		Xe_Fatal(xe, "ucode1.bin: %m");
	fread(xe->ucode1, 0x900*4, 1, f);
	fclose(f);

	Xe_pSetup(xe, buffer_base, 0xC, xe->ucode0, xe->ucode1);
...
{
	unsigned long kernel_mem[3] = {0,0,0};
	int f = open("/proc/device-tree/memory/reg", O_RDONLY);
	if (f >= 0)
	{
		read(f, &kernel_mem, 12);
		close(f);
	}
	if (!kernel_mem[2])
		Xe_Fatal(xe, "couldn't determine memory size\n");

	printf("linux kernel uses %ld MB\n", kernel_mem[2] / 1024 / 1024);
	
	if (kernel_mem[2] > RINGBUFFER_BASE)
		Xe_Fatal(xe, "sorry, physical memory to use is blocked by kernel.\n");

	xe->regs = ioremap(0xec800000ULL, 0x20000, 1);
	if (!xe->regs)
		Xe_Fatal(xe, "ioremap failed - %m");
	xe->rb = xe->rb_primary = ioremap(RINGBUFFER_BASE, RINGBUFFER_SIZE, 0);
	
// optimize framebuffer to the end, so we have a bit more space: w32(0x6110, 0x1fc00000);
	...
	xe->tex_fb.bypp = 4;
	xe->tex_fb.base = (void*)ioremap(xe->tex_fb.ptr, xe->tex_fb.height * xe->tex_fb.pitch, 0);
...
void Xe_VBBegin(struct XenosDevice *xe, int pitch)
{
	if (xe->vb_head || xe->vb_current)
		Xe_Fatal(xe, "FATAL: VertexBegin without VertexEnd!\n");
